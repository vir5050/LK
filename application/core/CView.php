...
class CView
{
	...
	public function renderContent($view)
	{
        if($this->_isComRendered){
			CDebug::addMessage('warnings', 'render-double-call', My::t('core', 'Double call of function {function}', array('{function}'=>'renderContent()')));
			return '';
		}
        ...
        if(isset($trace[1])){
            ...
            if($calledByClass != '' && $calledByClass != 'CComponent'){
				CDebug::addMessage('errors', 'render-content-caller', My::t('core', 'The View::renderContent() method cannot be called by {class} class. Component classes allowed only.', array('{class}'=>$calledByClass)));
                return false;                
            }
        }
        ...
        if(is_file($this->__viewFile) && file_exists($this->__viewFile)){
            ...
            if(file_exists($this->__viewFile)){
                ...
                if(preg_match('/[A-Z]/', $this->_controller)){
                    CDebug::addMessage('errors', 'renderContent-view', My::t('core', 'The system is unable to find the requested view file: {file}. Case sensitivity mismatch!', array('{file}'=>$this->__viewFile)));
                }else{
                    CDebug::addMessage('errors', 'renderContent-view', My::t('core', 'The system is unable to find the requested view file: {file}', array('{file}'=>$this->__viewFile)));
                }
            }

            echo $this->_content;
        }else{
            CDebug::addMessage('errors', 'component-render-view', My::t('core', 'The system is unable to find the requested component view file: {file}', array('{file}'=>$this->__viewFile)));
        }
	...
	{
        if($this->_isRendered){
			CDebug::addMessage('warnings', 'render-double-call', My::t('core', 'Double call of function {function} with parameters: {params}', array('{function}'=>'render()', '{params}'=>$params)));
			return '';
		}
        $trace = debug_backtrace();
        if(isset($trace[1])){
            ...
            if($calledByClass != '' && $calledByClass != 'CController'){
				CDebug::addMessage('errors', 'render-caller', My::t('core', 'The View::render() method cannot be called by {class} class. Controller classes allowed only.', array('{class}'=>$calledByClass)));
                return false;                
            }
        }
			...
			if(APP_MODE == 'test'){
				...
				if(!file_exists($this->__template)){
					...
					if(!empty($this->_template)){
						CDebug::addMessage('errors', 'render-template', My::t('core', 'Template file: "templates/{template}" cannot be found.', array('{template}'=>$this->_template)));
					}
				}
                $this->__viewFile = APP_PATH.DS.'protected'.DS.'views'.DS.$this->__controller.DS.$this->__view.'.php';
				if(!is_file($this->__viewFile)){
                    $this->__viewFile = APP_PATH.DS.'protected'.DS.My::app()->mapAppModule($this->__controller).'views'.DS.$this->__controller.DS.$this->__view.'.php';
                }
                ...
                if(file_exists($this->__viewFile)){
					...
					if(preg_match('/[A-Z]/', $this->_controller)){
						CDebug::addMessage('errors', 'render-view', My::t('core', 'The system is unable to find the requested view file: {file}. Case sensitivity mismatch!', array('{file}'=>$this->__viewFile)));
					}else{
						CDebug::addMessage('errors', 'render-view', My::t('core', 'The system is unable to find the requested view file: {file}', array('{file}'=>$this->__viewFile)));						
					}
                }
				...
				if($isPartial){
					...
					if($this->__isTemplateFound){
						...
						My::app()->getClientScript()->render($this->__templateContent);
