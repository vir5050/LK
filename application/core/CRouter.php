...
class CRouter
{
    ...
    public function route()
    {
        ...
        if(is_file($appDir.$file)){
            ...
            $comDir = APP_PATH.DS.'protected'.DS.My::app()->mapAppModule($this->_controller).'controllers'.DS;
            if(is_file($comDir.$file)){
                $class = $this->_controller.'Controller';
            }else{
                $class = 'ErrorController';
                My::app()->setResponseCode('404');
                CDebug::addMessage('errors', 'controller', My::t('core', 'Router: unable to resolve the request "{controller}".', array('{controller}' => $this->_controller)));
            }
        }
        ...
        if(is_callable(array($controller, $this->_action.'Action'))){
            ...
            CDebug::addMessage('errors', 'action', My::t('core', 'The system is unable to find the requested action "{action}".', array('{action}' => $this->_action)));
        }else{
            $action = 'indexAction';
        }
