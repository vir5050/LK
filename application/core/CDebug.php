...
class CDebug
{
    ...
    public static function addMessage($type = 'params', $key = '', $val = '', $storeType = '')
    {
        ...
        if($storeType == 'session'){
            My::app()->getSession()->set('debug-'.$type, $val);
        }
    ...
    public static function displayInfo()
    {
        ...
        if($debugError = My::app()->getSession()->get('debug-errors')){
            My::app()->getSession()->remove('debug-errors');
        }
        if($debugWarning = My::app()->getSession()->get('debug-warnings')){
            My::app()->getSession()->remove('debug-warnings');
        }
		...
		<div id="debug-panel">
			...
			<ul class="debug-tabs">
				<li tabId="debug-general" class="handle-debug-tab active"><a href="javascript:changeDebugTab(\'debug-general\')"><i data-icon="&#xe023;"></i><label>> '.My::t('core', 'General').'</label></a></li>
				<li tabId="debug-params" class="handle-debug-tab"><a href="javascript:changeDebugTab(\'debug-params\')"><i data-icon="&#xe205;"></i><label>> '.My::t('core', 'Params').'</label></a></li>
				<li tabId="debug-warnings" class="handle-debug-tab"><a href="javascript:changeDebugTab(\'debug-warnings\')"><i data-icon="&#xe036;"></i><label>> '.My::t('core', 'Warnings').'</label></a></li>
				<li tabId="debug-errors" class="handle-debug-tab"><a href="javascript:changeDebugTab(\'debug-errors\')"><i data-icon="&#xe20a;"></i><label>> '.My::t('core', 'Errors').'</label></a></li>
				<li tabId="debug-sql-queries" class="handle-debug-tab"><a href="javascript:changeDebugTab(\'debug-sql-queries\')"><i data-icon="&#xe0be;"></i><label>> '.My::t('core', 'SQL Queries').'</label></a></li>
				<li tabId="debug-server" class="handle-debug-tab"><a href="javascript:changeDebugTab(\'debug-server\')"><i data-icon="&#xe0aa;"></i><label>> '.My::t('core', 'Server responses').'</label></a></li>
			</ul>
		    ...
		    <div id="debug-general" class="debug-content active">
			    '.My::t('core', 'Total running time').': '.round((float)self::$_endTime - (float)self::$_startTime, 6).' '.My::t('i18n', 'time.abbreviated.seconds').'<br />
			    '.My::t('core', 'Memory usage').': '.self::getMemoryUsage().'<br />
			    '.My::t('core', 'Version').': '.My::getVersion().'<br /><br />';
    ...
    public static function shortInfo()
    {
        ...
        $content = ' <span>'.My::t('core', 'Time').':</span> '.round((float)self::$_endTime - (float)self::$_startTime, 3).' '.My::t('i18n', 'time.abbreviated.seconds').'.';
        $content .= ' <span>'.My::t('core', 'Memory').':</span> '.self::getMemoryUsage();
        return $content;
    }
