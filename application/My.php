...
class My
{
    ...
    private static $_coreComponents = array(
        ...
        'coreMessages' => array('class' => 'CMessageSource', 'language' => 'ru'),
            ...
            if(isset(self::$_classMap[$pureClassType])){
                ...
                if(is_file($classFile)){
                    ...
                    if(is_file($classFile)){
                        ...
                        CDebug::addMessage('errors', 'missing-model', My::t('core', 'Unable to find class "{class}".', array('{class}'=>$className)), 'session');
    ...
    public function attachEventHandler($name, $handler)
    {
        if($this->_hasEvent($name)){
            ...
            CDebug::addMessage('errors', 'events-attach', My::t('core', 'Event "{class}.{name}" is not defined.', array('{class}'=>get_class($this), '{name}'=>$name)));
        }
    }
    ...
    public function detachEventHandler($name)
    {
        if($this->_hasEvent($name)){
            ...
            CDebug::addMessage('errors', 'events-detach', My::t('core', 'Event "{class}.{name}" is not defined.', array('{class}'=>get_class($this), '{name}'=>$name)));
        }
    }
        ...
        if(isset($this->_events[$name])){
            foreach($this->_events[$name] as $handler){
                if(is_string($handler[1])){
                    ...
                    CDebug::addMessage('errors', 'events-raising', My::t('core', 'Event "{{class}}.{{name}}" is attached with an invalid handler "{'.$handler[1].'}".', array('{class}'=>$handler[0], '{name}'=>$handler[1])));
                }
            }
        }
    }
