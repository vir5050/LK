...
class CAuth
{
    ...
    {
        return (My::app()->getSession()->get('loggedIn') == true) ? true : false;
    }
    ...
    public static function handleLogin($location = '')
    {
        ...
        if(!self::isLoggedIn()){
            if ($location == 'index/index') {
				header('location: '.My::app()->getRequest()->getBaseUrl());
				exit;
			}
			
			header('location: '.My::app()->getRequest()->getBaseUrl().$location);
            exit;
        }
    }
    ...
    {
        return (self::isLoggedIn()) ? My::app()->getSession()->get('loggedId') : null;
    }
    ...
    {
        return (self::isLoggedIn()) ? My::app()->getSession()->get('loggedName') : null;
    }
    ...
    {
        return (self::isLoggedIn()) ? My::app()->getSession()->get('loggedEmail') : null;
    }
    ...
    {
        return (self::isLoggedIn()) ? My::app()->getSession()->get('loggedRole') : null;
    }
    ...
    {
        $result = null;
        if(self::isLoggedIn() && My::app()->getSession()->isExists($param)){
            $result = My::app()->getSession()->get($param);
        }
        return $result;
    }
}