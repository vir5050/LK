...
class CTime
{
    ...
    public static function makePretty($ts, $type = 'wide', $showTime = true)
    {
        if(!empty($ts)){
            ...
            $time = $dt->format('j');
            $time .= self::S.My::t('i18n', 'monthNames.'.$type.'.'.$dt->format('n'));
            $time .= self::S.$dt->format('Y');
            if($showTime){
                $time .= self::S.My::t('core', 'in').self::S.$dt->format('G:i');
            }
        }else{
            $time = My::t('core', 'Failed');
        }
    ...
    public static function convertSecondsToTime($seconds = 0)
    {
        if($seconds > 0){
            ...
            $time = $dtf->diff($dtt)->format(
                '%a '.My::t('i18n', 'time.abbreviated.days')
                .'. %h '.My::t('i18n', 'time.abbreviated.hours')
                .'. %i '.My::t('i18n', 'time.abbreviated.minutes')
                .'. %s '.My::t('i18n', 'time.abbreviated.seconds')
            );
        }else{
            $time = My::t('core', 'none');
        }
