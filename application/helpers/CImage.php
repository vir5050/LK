...
class CImage
{
    ...
    public static function resizeImage($imagePath, $imageName, $resizeWidth = '', $resizeHeight = '')
	{
        ...
        if(empty($imagePathName)){
            CDebug::addMessage('errors', My::t('core', 'Uploaded {file} is not a valid image! Please check carefully the file type.', array('{file}'=>$imageName)));
		}else if(!function_exists('imagecreatefromjpeg')){
            CDebug::addMessage('errors', My::t('core', 'The function {function} does not exist! Please check carefully your server settings.', array('{function}'=>'imagecreatefromjpeg')));
			return $imageName;
        }else{
			if($imagePathName){
