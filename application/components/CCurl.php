...
class CCurl extends CComponent
{
    ...
    function __construct()
    {
        if(!function_exists('curl_init')){
            CDebug::addMessage('errors', 'curl', My::t('core', 'You must have CURL enabled in order to use Curl extension.'));
        }
    }
    ...
    {
        if(!is_array($values)){
            CDebug::addMessage('errors', 'curl', My::t('core', 'Options must be an array.'));
        }else{
    ...
    protected function setDefaults()
    {
        ...
        if(!isset($this->options['setOptions'][CURLOPT_ENCODING])){
            $this->setOption(CURLOPT_ENCODING, My::app()->charset);
        }
    ...
    private function handleCurl($url, array $postData)
    {
        ...
        if(!$this->url){
            CDebug::addMessage('errors', 'curl', My::t('core', 'You must set Url.'));
        }
        ...
        if(empty($postData)){
            ...
            if(!is_array($postData)){
                CDebug::addMessage('errors', 'curl', My::t('core', '$postData must be an array type.'));
            }
            $this->setOption(CURLOPT_URL, $this->url);
            $this->setDefaults();
        ...
        if(isset($this->options['proxylogin'])){
            if(!isset($this->options['proxy'])){
                CDebug::addMessage('errors', 'curl', My::t('core', 'If you use "proxylogin", you must define "proxy" with arrays.'));
            }else{
        ...
        if($this->data === false or $this->data === null){
            ...
            CDebug::addMessage('errors', 'curl', My::t('core', 'Error code - {code}. Error message - {message}. '.$url, array('{code}'=>$this->error_code, '{message}'=>curl_error($this->ch))));
        }else{
    ...
    public function run($url, array $postData = array())
    {
        if(!CValidator::isUrl($url)){
            CDebug::addMessage('errors', 'curl', My::t('core', 'The parameter $url must be a valid URL string value!'));
            return false;
        }
