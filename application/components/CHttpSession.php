...
class CHttpSession extends CComponent
{
    ...
    private function _openSession()
    {
        ...
        if(APP_MODE == 'debug' and session_id() == ''){
            CDebug::addMessage('errors', 'session', My::t('core', 'Failed to start session'));
        }
    }
    ...
    private function _setCookieMode($value = '')
    {
        if($value === 'none'){
            ...
            CDebug::addMessage('warnings', 'session_cookie_mode', My::t('core', 'HttpSession.cookieMode can only be "none", "allow" or "only".'));
        }
    }
}