...
class CHttpRequest extends CComponent
{
    ...
    public function setBaseUrl($absolute = true)
    {
        ...
        if(basename($_SERVER['SCRIPT_NAME']) === $scriptName){
            ...
            CDebug::addMessage('error', 'entry_script', My::t('core', 'Framework is unable to determine the entry script URL.'));
        }
    ...
    public function getCsrfTokenValue()
    {
        if($this->_csrfTokenValue === null){
            ...
            My::app()->getSession()->set('token', $this->_csrfTokenValue);
        }
    ...
    public function getCaptchaValue()
    {
        if($this->_captchaValue === null){
            ...
            My::app()->getSession()->set('captcha', $this->_captchaValue);
        }
    ...
    public function validateCsrfToken()
    {
        if($this->isPostRequest()){
            if(My::app()->getSession()->isExists('token') && isset($_POST[$this->_csrfTokenKey])){
                $tokenFromSession = My::app()->getSession()->get('token');
            ...
            if(!$valid and $mustValid===true){
                unset($_POST); 
                CDebug::addMessage('errors', 'csrf_token', My::t('core', 'Security error occurred.'));
            }
        }
    }
*/
    ...
    protected function _cleanRequest()
    {
        ...
        if($this->getCsrfValidation()) My::app()->attachEventHandler('_onBeginRequest', array($this, 'validateCsrfToken'));
    }
