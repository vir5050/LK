...
class CDatabase extends PDO
{
    ...
    public function __construct($params = array())
    {
        if(!empty($params)){
            ...
            try{
                ...
                if(APP_MODE == 'debug'){
                    $output = str_ireplace('{DESCRIPTION}', '<p>'.My::t('core', 'This application is currently experiencing some database difficulties').'</p>', $output);
                    $output = str_ireplace(
                        '{CODE}',
                    ...
                    $output = str_ireplace('{DESCRIPTION}', '<p>'.My::t('core', 'This application is currently experiencing some database difficulties. Please check back again later').'</p>', $output);
                    $output = str_ireplace('{CODE}', My::t('core', 'For more information turn on debug mode in your application'), $output);
                }
                echo $output;
                exit(1);
            }
    ...
    public function select($sql, $params = array(), $fetchMode = PDO::FETCH_ASSOC, $cacheId = '')
    {
        ...
        CDebug::addMessage('queries', ++self::$count.'. select | '.My::t('core', 'total').': '.(($result) ? count($result) : '0 (<b>'.($error ? 'error' : 'empty').'</b>)'), $sql);
        return $result;
    }
    ...
    public function update($table, $data, $where = '1')
    {
        ...
        CDebug::addMessage('queries', ++self::$count.'. update | '.My::t('core', 'total').': '.(($result) ? $sth->rowCount() : '0 (<b>error</b>)'), $sql);
        return $result;
    }
    ...
    public function delete($table, $where = '', $params = array())
    {
        ...
        CDebug::addMessage('queries', ++self::$count.'. delete | '.My::t('core', 'total').': '.(($result) ? $result : '0 (<b>error</b>)'), $sql);
        return $result;
    }
    ...
    public function customQuery($sql, $fetchMode = PDO::FETCH_ASSOC)
    {
        ...
        CDebug::addMessage('queries', ++self::$count.'. query | '.My::t('core', 'total').': '.(($result) ? count($result) : '0 (<b>error</b>)'), $sql);
        return $result;
    }
    ...
    public function customExec($sql)
    {
        ...
        CDebug::addMessage('queries', ++self::$count.'. query | '.My::t('core', 'total').': '.(($result) ? $result : '0 (<b>error</b>)'), $sql);
        return $result;
    }
