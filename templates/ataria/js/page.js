...
var CharmBar = {
    roles: function(el) {
        ...
        ajax.send('POST', 'account/ajax', {act:'get_roles'}, function() {
            ...
            try {
                ...
                each(roles, function(k, v) {
                    var tr = ce('tr'),
                        ...
                        span = ce('span', {class:'role-avatar role-cls s25 cls-' + v.cls});

                    a.innerHTML = '<span>' + v.name + '</span>';
            ...
            } catch(e) { table.innerHTML = 'Персонажы не найдены'; console.log(e); }
        });
    },
        ...
        ajax.send('POST', 'store/ajax', params, function() {
            ...
            try {
                ...
                var lt = ce('div', {class:'left-float'}, {marginTop:3, fontSize:14, lineHeight:34, color:'rgb(255, 255, 0)', textShadow:'1px 1px 0 rgb(0, 0, 0)'}),
                    bb = ce('button', {class:'button blue-gradient right-float'}),
                    bc = ce('button', {class:'button close right-float'});
