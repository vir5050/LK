 ...
 * select                                               _interpolateQuery
 * insert                                               _prepareParams
 * update
 * delete
 * customQuery
 * customExec
...
class CDatabase extends PDO
{
    ...
    public function __construct($params = [])
    {
        if(!empty($params)){
            ...
            try{
                @parent::__construct($dbDriver.':host='.$dbHost.';dbname='.$dbName, $dbUser, $dbPassword, [PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \''.$dbCharset.'\'']);
            ...
            try{
                if(CConfig::get('db') != ''){
                    @parent::__construct(CConfig::get('db.driver').':host='.CConfig::get('db.host').';dbname='.CConfig::get('db.database'),
                        ...
                        [PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \''.CConfig::get('db.charset', 'utf8').'\'']
                    );
                    $this->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                }else{
     ...
     * @internal param array $array parameters to bind
    ...
    {
        if(APP_MODE == 'demo'){
            self::$_errorMessage = 'This operation is blocked in Demo Mode!';
            return false;
        }
    ...
    /**
     * Returns data error page content
     * @return HTML code
     */
    private static function _fatalErrorPageContent()
    {
        ...
        <style type="text/css">
            ...
            body{background:#fff; color:#333; font-family:sans-serif; margin:2em auto; padding:1em 2em 2em; border:1px solid #dfdfdf; max-width:750px; text-align:left;}
